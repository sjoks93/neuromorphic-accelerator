...
int main(void)
{
    ...
    CHECK(nmc_core_add_output_destination_lut_entry(&core, 1, 0));
    CHECK(nmc_core_add_output_destination_lut_entry(&core, 2, 0));
    CHECK(nmc_core_add_output_destination_lut_entry(&core, 3, 1));

    CHECK(nmc_core_set_output_lut_range(&core, OUTPUT_X, 0, 2));
    CHECK(nmc_core_set_output_lut_range(&core, OUTPUT_Y, 2, 3));
